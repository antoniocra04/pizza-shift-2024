import type { PizzaIngredientInput, PizzaSizeInput } from '@api/__generated__/graphql';
...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    editProduct: (state, action: PayloadAction<CartPizza>) => {
      const { count, currentSize, ingredients, selectedToppings, totalPrice, name } =
        action.payload;
      ...
      newProducts[index].selectedIngredients = newProducts[index].selectedIngredients.filter(
    ...
    updateTotalCartPrice: (state) => {
      ...
      state.products.forEach((product) => {
        totalCartPrice +=
          calculateTotalPrice(product.currentSize, product.selectedToppings) * product.count;
      });
      state.cartTotalPrice = totalCartPrice;
    }
  }
